...
function FloatingShapes() {
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      <div className="absolute w-40 h-40 rounded-full animate-float-a" style={{ left: '8%', top: '20%', background: 'radial-gradient(circle,rgba(127,178,174,0.5),transparent 70%)', filter: 'blur(8px)' }} />
      <div className="absolute w-52 h-52 rounded-full animate-float-b" style={{ right: '6%', top: '30%', background: 'radial-gradient(circle,rgba(226,114,91,0.4),transparent 70%)', filter: 'blur(10px)' }} />
      <div className="absolute w-24 h-24 animate-wobble" style={{ left: '18%', bottom: '18%', border: `2px solid ${C.seaGlass}`, borderRadius: '30%', opacity: 0.4 }} />
      <div className="absolute w-16 h-16 rounded-full animate-float-a" style={{ right: '20%', bottom: '22%', border: `2px solid ${C.ember}`, opacity: 0.5 }} />
    </div>
  )
}
...
function EasingCard({ e }: { e: (typeof EASES)[number] }) {
  return (
    <motion.div variants={rise} className="rounded-2xl p-6" style={{ background: C.panel, border: `1px solid ${C.border}` }} whileHover={{ y: -6 }} transition={{ duration: 0.4, ease: EASE }}>
      <span className="text-xs uppercase tracking-widest" style={{ color: C.seaGlass }}>{e.name}</span>
      <svg viewBox="0 0 120 60" className="w-full h-14 my-4" preserveAspectRatio="none">
        <motion.path d={e.path} fill="none" stroke={C.seaGlass} strokeWidth="2.5" initial={{ pathLength: 0 }} whileInView={{ pathLength: 1 }} viewport={{ once: true }} transition={{ duration: 1, ease: EASE }} />
      </svg>
      <div className="relative h-4 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
        <span className="absolute top-1/2 -translate-y-1/2 w-4 h-4 rounded-full" style={{ background: C.ember, animation: `ease-run 1.6s ${e.css} infinite alternate` }} />
      </div>
    </motion.div>
  )
}
...
function Timeline() {
  return (
    <div className="relative rounded-2xl p-5 md:p-7 overflow-hidden" style={{ background: C.panel, border: `1px solid ${C.border}` }}>
      <div className="absolute top-4 bottom-4 w-px z-10" style={{ background: C.ember, left: '4%', animation: 'playhead-run 4s ease-in-out infinite alternate' }}>
        <span className="absolute -top-1 -translate-x-1/2 w-2.5 h-2.5 rotate-45" style={{ background: C.ember }} />
      </div>
      {TRACKS.map((t, r) => (
        <div key={t} className="flex items-center gap-4 py-3 border-b last:border-0" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
          ...
          <div className="relative flex-1 h-4">
            ...
            {[0.12, 0.38, 0.6, 0.85].map((p, k) => (
              <span key={k} className="absolute top-1/2 w-3 h-3" style={{ left: `${p * 100}%`, background: k % 2 ? C.seaGlass : C.ember, animation: `kf-pulse 2.4s ease-in-out ${(r + k) * 0.2}s infinite`, transformOrigin: 'center' }} />
            ))}
          </div>
        </div>
      ))}
...
const LAYERS = [
  { n: 'Заголовок', c: C.ember },
  { n: 'Логотип', c: C.seaGlass },
  { n: 'Фигуры', c: '#A8C9C5' },
  { n: 'Камера', c: C.deepOcean },
  { n: 'Null · контроллер', c: C.petrol },
  { n: 'Аудио', c: '#0B3A40' },
]
    ...
    <div className="rounded-2xl overflow-hidden" style={{ background: C.panel, border: `1px solid ${C.border}` }}>
      ...
      <div className="grid md:grid-cols-[230px_1fr]">
        ...
        <div className="border-b md:border-b-0 md:border-r py-2" style={{ borderColor: C.border }}>
          <div className="px-4 py-1.5 text-[10px] uppercase tracking-widest" style={{ color: C.petrol }}>Слои</div>
          {LAYERS.map((l, i) => (
            <div key={l.n} className="group flex items-center gap-2.5 px-4 py-2 cursor-default transition-colors hover:bg-white/5">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={C.petrol} strokeWidth="2"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7-11-7-11-7z" /><circle cx="12" cy="12" r="3" /></svg>
        ...
        <div className="relative flex items-center justify-center" style={{ minHeight: 240 }}>
          <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: 'linear-gradient(rgba(127,178,174,0.05) 1px,transparent 1px),linear-gradient(90deg,rgba(127,178,174,0.05) 1px,transparent 1px)', backgroundSize: '28px 28px' }} />
          <motion.div
            animate={{ x: [-70, 70, 70, -70], y: [0, -20, 20, 0], scale: [1, 1.4, 0.85, 1], rotate: [0, 90, 220, 360], borderRadius: ['24%', '50%', '24%', '24%'] }}
            transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut' }}
            className="w-20 h-20"
            style={{ background: `linear-gradient(135deg,${C.ember},${C.seaGlass})` }}
          />
          <motion.span
            className="absolute w-14 h-14 rounded-full border-2"
            style={{ borderColor: C.seaGlass }}
  ...
  return (
    <main data-screen="motion" className="animate-screen-in relative font-sans" style={{ background: C.abyss, color: C.chalk }}>
      ...
      <section className="px-6 md:px-12 py-24">
        <Reveal>
          <h2 className="font-bold uppercase tracking-tight mb-3" style={{ fontSize: 'clamp(1.8rem,5vw,3.5rem)', color: C.chalk, ...DISPLAY }}>Всё в тайминге</h2>
      ...
      <section className="px-6 md:px-12 py-24">
        <Reveal><h2 className="font-bold uppercase tracking-tight mb-3" style={{ fontSize: 'clamp(1.8rem,5vw,3.5rem)', color: C.chalk, ...DISPLAY }}>Рабочий процесс</h2></Reveal>
