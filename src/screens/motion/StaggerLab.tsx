    ...
    <section id="m-stagger" className="px-6 md:px-12" style={{ paddingBlock: 'var(--section-y)' }}>
      <div className="max-w-5xl">
        <SectionHead
          ...
          lead="Шаг между элементами создаёт ощущение композиции, но платит за него время. Считать надо не шаг, а всю волну: последний элемент трогается на шаге, умноженном на количество интервалов. Пока волна укладывается в 0.6 секунды, это ритм. Дальше — ожидание, которое посетитель списывает на медленный сайт."
        />
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
            ...
            {/* Множитель — UNITS−1, а не UNITS: первый элемент стартует без
                задержки, и волна кончается, когда трогается последний.
                В разделе про проверяемую арифметику ошибка на единицу
                стоила бы доверия ко всем остальным числам экрана. */}
            <span
              className="text-[11px] tabular-nums tracking-[0.14em]"
              style={{ color: 'var(--m-chalk)', ...MONO }}
              aria-live="polite"
            >
              {UNITS - 1} × {Math.round(step * 1000)} = {Math.round(total * 1000)} мс
            </span>
          </div>
