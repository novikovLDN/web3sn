...
/** Пул частиц: пыль от шагов/приземления и брызги воды. */
export default function Particles({ apiRef }: { apiRef: React.MutableRefObject<ParticleHandle | null> }) {
  const ref = useRef<THREE.Points>(null)
  const state = useRef(
    Array.from({ length: MAX }, () => ({ life: 0, max: 1, x: 0, y: 0, z: 0, vx: 0, vy: 0, vz: 0 }))
  )
  ...
  apiRef.current = {
    burst: (x, y, z, color, power) => {
      _c.set(color)
      const n = Math.floor(6 + power * 20)
      let spawned = 0
      for (let i = 0; i < MAX; i++) {
        const p = state.current[i]
        if (p.life > 0) continue
        const a = Math.random() * Math.PI * 2
        const sp = 0.7 + Math.random() * 2.4 * power
  ...
  return (
    <points ref={ref}>
      ...
      <pointsMaterial size={0.2} vertexColors transparent opacity={0.9} depthWrite={false} sizeAttenuation />
    </points>
  )
}
